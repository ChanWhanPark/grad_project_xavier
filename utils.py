...
def decode_segmap(label_mask, dataset, plot=False, **kwargs):
    ...
    #print('label_mask = {}'.format(label_mask[511]))
    #print('label_mask ==5  = {}'.format(label_mask==5))
    #print('label_mask[0][511]  = {}'.format(label_mask[511][1023]))
    #print('label_mask.shape  = {}'.format(label_mask.shape))
    #print('np.where(label_mask[511][*])  = {}'.format(np.where(label_mask[511] == 5 )))
    #print('np.min(np.where(label_mask[511][*])  = {}'.format(np.min(np.where(label_mask[511] == 5 ))))
    #print('np.max(np.where(label_mask[511][*])  = {}'.format(np.max(np.where(label_mask[511] == 5 ))))
    r = label_mask.copy()
    g = label_mask.copy()
    b = label_mask.copy()
    ...
    rgb[:, :, 2] = b / 255.0
    try:
        a = np.max(np.where(label_mask[120] == 5 ))
        c = np.max(np.where(label_mask[500] == 5 ))
        b = np.min(np.where(label_mask[120] == 5 ))
        d = np.min(np.where(label_mask[500] == 5 ))
    except:
        a= 0
        b=0
        c=1
