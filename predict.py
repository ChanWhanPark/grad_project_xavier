...
## RUN OPTIONS ###
MODEL_PATH = "/home/aeslab/다운로드/model_iou_77.pth.tar"
#MODEL_PATH = "/home/aeslab/workspace/segmentation-selectstar/run/surface/deeplab/model_best.pth.tar"
ORIGINAL_HEIGHT = 720
ORIGINAL_WIDTH = 1280
MODEL_HEIGHT = 512
MODEL_WIDTH = 1024
...
class FrameGeneratorMP4:
    def __init__(self, mp4_file, output_path=None, show=True):
...
class ModelWrapper:
    ...
    def predict(self, rgb_img: np.array):
        ...
        #segmap,dox1,dox2,dox3,dox4 = decode_segmap(pred, dataset='custom', label_colors=CUSTOM_COLOR_MAP, n_classes=CUSTOM_N_CLASSES)
        segmap = decode_segmap(pred, dataset='custom', label_colors=CUSTOM_COLOR_MAP, n_classes=CUSTOM_N_CLASSES)
        #print('segmap : {}'.format(segmap.shape))
...
def main():
    ...
    for index, img in enumerate(generator):
        ...
        #segmap = model_wrapper.predict(generator)
        #cv2.imshow('ds', segmap)
        #cv2.waitKey(0)
        if OVERLAPPING:
